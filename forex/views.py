...
from .models import Account, Trade, ForexPair, OptionTrade, CryptoPair, CryptoTrade
...
def options(request, symbol):
    ...
    demo_account = Account.objects.get(user=request.user, account_type='DEMO')
    demo_account_balance = demo_account.balance
    get_forex_historical_data(request, symbol)
...
def update_trade_outcomes(request):
    ...
    for trade in trades:
        ...
        try:
            ...
            if any(point['time'] == expiry_time for point in forex_pair.chart_data):
                # Find the corresponding closing price
                closing_price = next(point['value'] for point in forex_pair.chart_data if point['time'] == expiry_time)
